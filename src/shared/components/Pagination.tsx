...
export function Pagination({
  ...
  return (
    <nav
      ...
      className={`flex flex-wrap items-center justify-between gap-2.5 py-3.5 sm:border-t sm:border-line sm:px-5 ${className}`}
    >
      <div className="hidden items-center gap-2.5 sm:flex">
        {infoText ? (
          typeof infoText === 'string' ? (
            <span className="text-[13px] text-muted">{infoText}</span>
          ) : (
            infoText
          )
        ) : (
